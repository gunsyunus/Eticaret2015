jQuery(document).ready(function() {
     ...
     jQuery('#billing').toggle(!this.checked);
  	}).change();

  jQuery(".phone").mask("(0999)9999999");   
...
jQuery(".topnav").accordion({
	...
	openedSign: '-'
			...
			jQuery("#cardpayment").hide();
		    jQuery("#c1").click(function(){ jQuery("#cardpayment").show(); });			
		    jQuery("#c2,#c3,#c4,#c5,#c6").click(function(){ jQuery("#cardpayment").hide(); });

jQuery("#city_id").change(function() {
  jQuery.getJSON("/county/"+jQuery("#city_id").val(), function(jsonData){  
    var countySelect = jQuery("#county_id");
    countySelect.empty();  
      jQuery.each(jsonData, function(i,data) {
        countySelect.append('<option value='+data.id_county+'>'+data.name+'</option>');            
      });  
  });
});

jQuery("#billing_city").change(function() {
  jQuery.getJSON("/county/"+jQuery("#billing_city").val(), function(jsonData){  
    var countySelect = jQuery("#billing_county");
    countySelect.empty();  
      jQuery.each(jsonData, function(i,data) {
        countySelect.append('<option value='+data.id_county+'>'+data.name+'</option>');            
      });  
...
jQuery.ajax({ type:'POST', url:'/newsletter', data:{email:email}, headers: {'X-CSRF-TOKEN': jQuery("input[name=_token]").val()}, 
error: function(data) { jQuery('#subscribe_msg').html(data.responseJSON.email); },
success:function(data) { jQuery('#subscribe_msg').html(data.message); jQuery("#subscribe").prop("disabled", true); } });
});
