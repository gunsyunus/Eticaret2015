(function($){
    $.fn.cloud_zoom = function(options){
        ...
        var o = $.extend({
            ...
            zoom_easing: false,                    // Ease the animation while moving the zoomed image (false/false)
            click_to_zoom: false,                // Will start zooming when image is clicked instead of when hovering (when false, click-callback functions are disabled) (false/false)
            zoom_element: 'auto',                // Supply your custom zoom area element as a CSS type selector ('auto'/'#selector'/'.selector')
            zoom_enable: false,
            show_descriptions: false,            // Shows the description area if a title attribute is given to the source image (false/false)
            ...
            smallthumb_hide_single: false,        // Don't show the small thumb when there is only 1 image (false/false)
            smallthumb_select_on_hover: false,    // This will scroll through the small thumbnails when hovering them, instead of clicking them (false/false)
            smallthumbs_position: 'bottom',        // Where to position the row of small thumbnails ('top' / 'bottom' / 'left' / 'right')
            show_begin_end_smallthumb: false,    // Whether to show extra thumbnails at the beginning and the end that navigate back to the end or beginning (false/false)
            magnifier_opacity: 0.5,                // Opacity of the magnifier area (does not apply if magnifier_invert is false) (number between or equal to 0-1)
            magnifier_invert: false,                // Make the large thumbnail clear through the magnifier, opaque around it (opacity is the value of magnifier_opacity) (false/false)
            show_icon: false,                    // Shows the icon image in the middle of the magnifier (only if magnifier_invert is false) and left-bottom of large thumb (false/false)
            icon_offset: 20,                    // The icon's offset to the left-bottom corner (value in pixels)
            hide_cursor: false,                    // Hides the cursor when hovering the large thumbnail (only works in some browsers) (false/false)
            show_hint: false,                    // Show "hint" until image is zoomed for the first time (false/false)
            hint_offset: 15,                    // The hint's offset to the right-top corner (value in pixels)
            speed: 600,                            // All animation speeds are based on this setting (value in ms)
            autoplay: false,                        // Makes the thumbs switch automatically when not interacting (at each autoplay_interval below) (false/false)
            autoplay_interval: 6000,            // Time showing each image if autoplay is on (value in ms)
            keyboard: false,                        // Left/right arrow keys to navigate (false/false)
            right_to_left: false,                // Makes the thumbnails slide from right to left and the caption texts RTL (false/false)
        ...
        $.each(this, function(){
            ...
            if($container.is('ul') && $container.children('li').length && $container.find('img.zoom_source_image').length){
...
// NEW ELEMENTS & CACHE
                $container.addClass('cloud_zoom1').show();
                var $thumbs;
                if($container.children('li').first().hasClass('zoom_img')){
                    $thumbs = $container.children('li.zoom_img');
                }else{
                    $thumbs = $container.children('li').addClass('zoom_img');
                }
                $container.children('li.zoom_img_active').hide().removeClass('zoom_img_active');
                $thumbs.first().show().addClass('zoom_img_active').css('opacity', 1);
                ...
                $.each($thumbs, function(i){
                    i += 1;
                    var $t = $(this),
                        $t_thumb = $t.find('.zoom_img_image').removeAttr('alt').show(),
                        $t_source = $t.find('.zoom_source_image'),
                        $t_anchor = $t.find('a');
                    $t.data('id', i).addClass('thumb_'+i);
                    // No thumb, but source
                    if(!$t_thumb.length && $t_source.length){
                        $t.prepend('<img class="zoom_img_image" src="'+$t_source.attr('src')+'" />');
                    }
                    ...
                    if($t_anchor.length){
                        $t.find('.zoom_img_image').data('anchor', $t_anchor.attr('href'));
                    }
                });
                var $thumb_images = $thumbs.find('.zoom_img_image').css({ width:o.thumb_image_width, height:o.thumb_image_height }).show();
                ...
                // Add magnifier
                var $magnifier;
                $container.children('li.zoom_magnifier').remove();
                $magnifier = $('<li class="zoom_magnifier"><div><img /></div></li>').appendTo($container);
                var $magnifier_img_area = $magnifier.children('div');
                var $magnifier_img = $magnifier_img_area.children('img');

                // Add zoom icon
                var $icon;
                $container.children('li.zoom_icon').remove;
                $icon  = $('<li class="zoom_icon">&nbsp;</li>').appendTo($container);
                if(o.show_icon){
                    $icon.show();
                }
                ...
                if(zoom_element!=='auto' && zoom_element && $(zoom_element).length){
                    $zoom = $(zoom_element)
                        .addClass('product_zoom_area')
                        .html('<div><img class="pr_zoom_img" /></div>');
                }else{
                    zoom_element = 'auto';
                    $container.children('li.product_zoom_area').remove();
                    $zoom = $('<li class="product_zoom_area"><div><img class="pr_zoom_img" /></div></li>').appendTo($container);
                }
                var $zoom_img_area = $zoom.children('div'),
                    $zoom_preview;
                if(preview){
                    $zoom_preview = $('<img class="product_zoom_preview" />').css({ width:o.source_image_width, height:o.source_image_height, opacity:0.3 }).prependTo($zoom_img_area).show();
                }
                var $zoom_img = $zoom_img_area.children('.pr_zoom_img').css({ width:o.source_image_width, height:o.source_image_height });

                // Add description area
                var $description;
                if(o.show_descriptions){
                    $description = $('<div class="zoom_description'+ ((o.right_to_left)?' rtl':'') +'"></div>').prependTo($zoom);
                }
                ...
                if(images > 1 || !o.smallthumb_hide_single){
                    $container.children('li.zoom_small_thumbs').remove();
                    $smallthumbs = $('<li class="zoom_small_thumbs"><ul></ul></li>').appendTo($container);
                    
                    $smallthumbs_ul = $smallthumbs.children('ul');
                    $.each($thumb_images, function(){
                        var $t = $(this);
                        src = $t.data('src');
                        thumb_id = $t.parents('.zoom_img').data('id');
                        $('<li><img class="zoom_small_thumb" src="'+src+'" /></li>').data('thumb_id', thumb_id).appendTo($smallthumbs_ul);
                    });
                    ...
                    if(images > small_thumbs){
                        ...
                        if(o.show_begin_end_smallthumb){
                            ...
                            $('<li class="zoom_smallthumb_first zoom_smallthumb_navtoend"><img class="zoom_small_thumb" src="'+src+'" /></li>')
                                .data('src', src)
                            ...
                            $('<li class="zoom_smallthumb_navtostart"><img class="zoom_small_thumb" src="'+src+'" /></li>')
                                .data('src', src)
                                .data('thumb_id', thumb_id)
                                .css({ opacity:o.smallthumb_inactive_opacity })
                            ...
                            $smallthumb.eq(1).addClass('zoom_smallthumb_active').css({ opacity:1 });
                        }else{
                            // Set first active smallthumb
                            $smallthumb.eq(0).addClass('zoom_smallthumb_first zoom_smallthumb_active').css({ opacity:1 });
                        }
                        // Prepare for moving them left/right / up/down
                        $smallthumb.eq(small_thumbs-1).addClass('zoom_smallthumb_last');
                    }
                    // Smallthumbs are within boundries
                    else{
                        // Set first active smallthumb
                        $smallthumb.eq(0).addClass('zoom_smallthumb_active').css({ opacity:1 });
                    }
                    // Apply id
...
// INITIATE FIRST RUN
                ...
                var $current_thumb = $thumbs.first().find('.zoom_img_image'),
                    $current_source = $thumbs.first().find('.zoom_source_image');
                if(o.magnifier_invert){
...
// FUNCTIONS
                ...
                var st_click = function($next_active, moved){
                    var $next_thumb,
                        image_number,
                        $active = $container.find('.zoom_smallthumb_active').removeClass('zoom_smallthumb_active');
                    $next_active.addClass('zoom_smallthumb_active');
                    ...
                    // Switch large thumb
                    $container.find('.zoom_img_active').removeClass('zoom_img_active').stop().animate({ opacity:0 }, o.speed, function(){
                        $(this).hide();
                    });
                    $next_thumb = $thumbs.filter('.thumb_'+$next_active.data('thumb_id')).addClass('zoom_img_active').show().stop().css({ opacity:0 }).animate({ opacity:1 }, o.speed);
                    $current_thumb = $next_thumb.find('.zoom_img_image');
                    $current_source = $next_thumb.find('.zoom_source_image');
                    // Switch magnifier
                    if(o.magnifier_invert){
                ...
                var prev = function(){
                    var $prev;
                    if(o.magnifier_invert){
                        $container.find('.zoom_img_active').mouseleave();
                    }
                    if(!o.right_to_left){
                        $prev = $container.find('.zoom_smallthumb_active').prev();
                        if(!$prev.length){
                            if(images > small_thumbs){
                                st_slide_to_end();
                            }else{
                        ...
                        $prev = $container.find('.zoom_smallthumb_active').next();
                        if(!$prev.length){
                            if(images > small_thumbs){
                                st_slide_to_start();
                            }else{
                ...
                var next = function(){
                    ...
                    if(!o.right_to_left){
                        $next = $container.find('.zoom_smallthumb_active').next();
                        if(!$next.length){
                            if(images > small_thumbs){
                                st_slide_to_start();
                            }else{
                        ...
                        $next = $container.find('.zoom_smallthumb_active').prev();
                        if(!$next.length){
                            if(images > small_thumbs){
                                st_slide_to_end();
                            }else{
                ...
                var show = function(number){
                    ...
                    if($number.length && !st_moving){
                        var $active = $container.find('.zoom_smallthumb_active'),
                            active = $active.data('id')-1,
                            difference;
                        // Move backward
                        if(active > number){
                            st_steps = active - number;
                            var $first = $container.find('.zoom_smallthumb_first'),
                        ...
                        // Move forward
                            ...
                            var $last = $container.find('.zoom_smallthumb_last'),
...
// CALLBACK FUNCTIONS
                ...
                var click_callback = function(image_anchor){
                    ...
                    // Legacy support
                    if(typeof zoom_click_callback === 'function'){
                        zoom_click_callback(image_anchor, instance_id);
                        return false;
                    }
                    return true;
                };
                ...
                var change_callback = function(image_number){
                    ...
                    if(o.change_callback(image_number, instance_id)){
                        // Legacy support
                        if(typeof zoom_change_callback === 'function'){
                            zoom_change_callback(image_number, instance_id);
                        }
                    }
                };
...
function st_shift_1(direction){
    ...
    st_moving = true;
    var $first = $container.find('.zoom_smallthumb_first').removeClass('zoom_smallthumb_first');
    var $last = $container.find('.zoom_smallthumb_last').removeClass('zoom_smallthumb_last');
    var $next_first, $next_last, $next_active, distance;
    if(direction === 'left'){
        $next_first = $first.prev().addClass('zoom_smallthumb_first');
        $next_last = $last.prev().addClass('zoom_smallthumb_last');
        $next_active = $first;
    }else{
        $next_first = $first.next().addClass('zoom_smallthumb_first');
        $next_last = $last.next().addClass('zoom_smallthumb_last');
        $next_active = $last;
    }
    // If more steps
    if(st_steps){
...
function st_slide_to_startend(to){
    ...
    if(to==='end'){
        $next_first = $smallthumb.eq(smallthumbs-small_thumbs).addClass('zoom_smallthumb_first');
        $next_last = $smallthumb.eq(smallthumbs-1).addClass('zoom_smallthumb_last');
        $next_active = $next_last;
        if($next_last.hasClass('zoom_smallthumb_navtostart')){
            $next_active = $next_last.prev();
        }
    }else{
        $next_first = $smallthumb.eq(0).addClass('zoom_smallthumb_first');
        $next_last = $smallthumb.eq(small_thumbs-1).addClass('zoom_smallthumb_last');
        $next_active = $next_first;
        if($next_first.hasClass('zoom_smallthumb_navtoend')){
    ...
    st_slide_to_startend('end');
                ...
                if(images > 1 || !o.smallthumb_hide_single){
                    ...
                    $smallthumb.click(function(){
                        ...
                        if(!$this.hasClass('zoom_smallthumb_active') && (!st_moving || st_steps)){

                            // Slide left / up
                            if($this.hasClass('zoom_smallthumb_first') && $this.prev().length){
                                st_shift_1('left');
                            }
                            // Shift to end
                            else if($this.hasClass('zoom_smallthumb_navtoend')){
                                st_slide_to_end();
                            }

                            // Slide right / down
                            else if($this.hasClass('zoom_smallthumb_last') && $this.next().length){
                                st_shift_1('right');
                            }
                            // Shift to beginning
                            else if($this.hasClass('zoom_smallthumb_navtostart')){
                                st_slide_to_start();
                            }
